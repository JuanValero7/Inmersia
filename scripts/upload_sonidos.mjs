...
let subiOk = 0, subiErr = 0, reutilizados = 0
const mediaRows = []

// Pre-fetch de slugs que ya están en Supabase (fuente=biblioteca)
const slugsBiblioteca = new Set(
  mp3Files
    .map(f => f.match(/^grupo_(\d+)_(.+)\.mp3$/i))
    .filter(m => m && manMap[parseInt(m[1], 10).toString()]?.fuente === 'biblioteca')
    .map(m => m[2])
)

const mediaExistente = {}
if (slugsBiblioteca.size > 0) {
  const { data: existing } = await supa
    .from('biblioteca_media').select('slug, id, url')
    .in('slug', [...slugsBiblioteca])
  existing?.forEach(m => { mediaExistente[m.slug] = m })
  const faltantes = [...slugsBiblioteca].filter(s => !mediaExistente[s])
  if (faltantes.length)
    console.warn(`  ⚠ ${faltantes.length} slug(s) de biblioteca no encontrados en Supabase — se subirán igual\n`)
}
...
for (const file of mp3Files) {
  ...
  // Si ya existe en Supabase como biblioteca, reutilizar sin re-subir
  if (mediaExistente[slug]) {
    console.log(`  ★ ${storageFile}  →  slug: "${slug}"  (biblioteca — sin re-subir)`)
    reutilizados++
    subiOk++
    continue
  }
...
if (mediaRows.length > 0) {
  const mediaDedup = Object.values(Object.fromEntries(mediaRows.map(r => [r.slug, r])))
  if (mediaDedup.length < mediaRows.length)
    console.log(`  ⚠ ${mediaRows.length - mediaDedup.length} slug(s) duplicado(s) consolidado(s)`)
  const BATCH = 50
  for (let i = 0; i < mediaDedup.length; i += BATCH) {
    const { error } = await supa
      .from('biblioteca_media')
      .upsert(mediaDedup.slice(i, i + BATCH), { onConflict: 'slug' })
    if (error) console.error(`  ✗ biblioteca_media: ${error.message}`)
  }
  console.log(`  ✓ ${mediaDedup.length} entradas en biblioteca_media`)
}
...
// Párrafos (paginado para superar el límite de 1000 de PostgREST)
const parrafos = []
{
  const PAGE = 1000
  let from = 0
...
for (const a of asigRows) {
  ...
  links.push({ parrafo_id: parId, media_id: mediaId, metadata: a.frase ? { texto_ref: a.frase } : {} })
}
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ✅  Carga completa — ${titulo}
      Subidos    : ${subiOk - reutilizados} archivos nuevos a Storage
      Biblioteca : ${reutilizados} reutilizados (sin re-subir)
      Media DB   : ${mediaRows.length} entradas nuevas
