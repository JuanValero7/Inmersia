...
// Anchos mínimos de la caja del spotlight (NO del viewport: vive en una
// columna `flex:3` que es ~60% del ancho útil de la página).
//   portada 300px de alto = 207px de ancho (aspect-ratio 210/305)
//   la fila "Ver detalle" + "Preview" no baja de 234px
// Con "También llegaron" al lado: 207 + 34 + 234 + 32 + 210 + 28 ≈ 745, más
// holgura para que la reseña no quede en una línea → 800.
// Debajo de eso la tira no se esconde: baja a una fila horizontal, para no
// perder la única forma de traer otra novedad al centro.
const NOV_ANCHO_LATERAL = 800
...
function NovedadesSpotlight({ novedades = [], idx, setIdx, onOpen, onPreview }) {
  ...
  const holgado  = ancho === 0 || ancho >= NOV_ANCHO_LATERAL
  const estrecho = ancho !== 0 && ancho < NOV_ANCHO_COMPACTO
  ...
  return (
    <div ref={cajaRef} style={{ display: 'flex', alignItems: 'stretch', gap: 32, padding: '16px 14px 26px', flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: '1.15', minWidth: 0, display: 'flex', gap: estrecho ? 22 : 34, alignItems: 'center' }}>
        ...
        <div style={{ flex: 1, minWidth: 0, height: holgado ? portadaH : 'auto', display: 'flex', flexDirection: 'column', gap: estrecho ? 8 : 10 }}>
          ...
          <div style={{ flexShrink: 0, fontWeight: 800, fontSize: estrecho ? 20 : 24, lineHeight: 1.15, color: ink,
          ...
          <div style={{ flexShrink: 0, display: 'flex', gap: 11, flexWrap: 'wrap' }}>
            <button onClick={(e) => onOpen(libro, e.currentTarget.getBoundingClientRect())}
              style={{ display: 'flex', alignItems: 'center', gap: 9, background: '#F2792A', color: '#fff',
                border: `2px solid ${ink}`, borderRadius: 999, padding: estrecho ? '11px 19px' : '13px 26px', fontWeight: 700, fontSize: estrecho ? 14 : 15,
            ...
            <button onClick={() => onPreview(libro)}
              style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fffdf8', color: ink,
                border: `2px solid ${ink}`, borderRadius: 999, padding: estrecho ? '11px 15px' : '13px 20px', fontWeight: 700, fontSize: 14,
      ...
      {otros.length > 0 && (
        <div style={holgado
          ? { flex: '.6', minWidth: 210, maxWidth: 320, height: 300, display: 'flex', flexDirection: 'column',
              gap: 14, borderLeft: `2px solid ${ink}1f`, paddingLeft: 26 }
          : { width: '100%', display: 'flex', flexDirection: 'column', gap: 10,
              borderTop: `2px solid ${ink}1f`, paddingTop: 14 }}>
          ...
          <div style={holgado
            ? { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 14, overflowY: 'auto', paddingRight: 4 }
